...
import com.ar7lab.cherieapp.databinding.FragmentMarketBinding
...
import com.ar7lab.cherieapp.ui.marketsearch.MarketSearchActivity
...
class MarketFragment : BaseFragment(R.layout.fragment_market) {
    ...
    private val stateObserver = Observer<MarketViewModel.ViewState> { state ->
                ...
                itemMarketplace {
                    ...
                    listener(object : MarketArtListener {
                        override fun onClick(art: Art) {
                            ...
                            val action =MarketFragmentDirections.actionMarketToMarketDetail(art)
    ...
    private fun addClickEvent() {
        ...
        binding.ivSearch.setOnDebouncedClickListener {
            ...
            if (viewModel.isSearchBoxShow.get()!!) {
                imm!!.showSoftInput(binding.etSearch, InputMethodManager.SHOW_IMPLICIT)
            } else {
                imm!!.hideSoftInputFromWindow(binding.etSearch.windowToken, 0)
            }*/

            //<editor-fold desc="Pass the type for get result based on fine and digital arts">
            val bundle = Bundle().apply {
                putSerializable(MARKET_SEARCH_KEY, viewModel.marketTypeSelected.get())
            }
            startActivity(Intent(requireContext(), MarketSearchActivity::class.java).apply { putExtras(bundle) })
            //</editor-fold>
        ...
        binding.ivNotification.setOnDebouncedClickListener {
            if (viewModel.isNeedSearch.get()!!) {
                viewModel.search("")
                viewModel.isNeedSearch.set(false)
            } else {
                val action =
                    MarketFragmentDirections
                        .actionMarketToNotification()
                navManager.navigate(action)
            }
        }
