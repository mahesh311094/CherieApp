 ...
 * */
@HiltViewModel
internal class MarketViewModel @Inject constructor(
    private val artRepository: ArtRepository,
    private val sharePreferencesManager: SharePreferencesManager
) :
    BaseViewModel<MarketViewModel.ViewState, MarketViewModel.Action>(ViewState()) {
    ...
    private suspend fun loadArt() {
        ...
        if (searchQuery.get()?.length in 1..2) {
            ...
            when (marketTypeSelected.get()) {
                ...
                MarketTypeEnum.NTF_ART -> {
                    sendAction(Action.LoadingNTFArtSuccess(arrayListOf()))
                }
            }
        } else {
            ...
            when (marketTypeSelected.get()) {
                //Load data from API calling When "Traditional Art" tab is selected
                MarketTypeEnum.TRADITIONAL_ART -> {
                    val response = artRepository.searchArts(
                        _page,
                        _limit,
        ...
        //Set empty text in search view after getting the search result
        searchText.set("")
...
//                sendAction(Action.LoadingCompletelySuccess())
    ...
    fun onDoneClicked(view: View, actionId: Int, event: KeyEvent?): Boolean {
        if (actionId == EditorInfo.IME_ACTION_DONE) {
            Timber.e("onDoneClicked ${searchText.get()}")
