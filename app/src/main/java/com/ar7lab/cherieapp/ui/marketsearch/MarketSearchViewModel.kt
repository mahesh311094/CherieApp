...
@HiltViewModel
internal class MarketSearchViewModel @Inject constructor(
    private val artRepository: ArtRepository,
    private val sharePreferencesManager: SharePreferencesManager
): BaseViewModel<MarketSearchViewModel.ViewState, MarketSearchViewModel.Action>(
    ViewState()
){
    ...
    private suspend fun loadArt() {
        ...
        when (marketTypeSelected.get()) {
            ...
            MarketTypeEnum.NTF_ART -> {
                ...
                val response = artRepository.searchArts(
                    _page, _limit, FILTER_NFT_ART, userId = sharePreferencesManager.userId, _query
                )
    ...
    /**
     * When click on done from keyboard it will gives the result
