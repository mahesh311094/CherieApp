 ...
 * */
@HiltViewModel
internal class StoreUpcomingDealsViewModel @Inject constructor(
    private val artRepository: ArtRepository,
    private val sharePreferencesManager: SharePreferencesManager
) :
    BaseViewModel<StoreUpcomingDealsViewModel.ViewState, StoreUpcomingDealsViewModel.Action>(ViewState()) {
    ...
    private suspend fun loadArt() {
        ...
        if (state.isRefreshing) {
            when (marketTypeSelected.get()) {
                MarketTypeEnum.TRADITIONAL_ART -> _traditionalArts.clear()
            ...
            MarketTypeEnum.TRADITIONAL_ART -> {
                ...
                response.data?.upcomingArts?.let {
                    ...
                    if (state.isLoadingMore && it.isEmpty()) {
                        ...
                        _traditionalArts.addAll(it)
                        totalResult.set(_traditionalArts.size.toString())
                        sendAction(Action.LoadingTraditionalArtSuccess(_traditionalArts))
                    }
                }
            }
...
//                sendAction(Action.LoadingCompletelySuccess())
    ...
    private fun updateNotifyArt(artId: String) {
        ...
        val artTraditionalUpcomingDeals = _traditionalArts.find { it.id == artId }
        artTraditionalUpcomingDeals?.let {
            it.notified = !it.notified
        }
        //Nft Upcoming Deals like
        val artNFTSourceUpcomingDeals = _nftArts.find { it.id == artId }
    ...
    internal sealed class Action : BaseAction {
        ...
        class LoadingTraditionalArtSuccess(val arts: ArrayList<Art>) : Action()
    ...
    override fun onReduceState(viewAction: Action): ViewState = when (viewAction) {
        ...
        is Action.LoadingTraditionalArtSuccess -> state.copy(
